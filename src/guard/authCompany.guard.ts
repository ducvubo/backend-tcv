...
export class AuthGuardCompany implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!cp_refresh_token || !cp_access_token) throw new HttpException('Token không hợp lệ', HttpStatus.UNAUTHORIZED)

    const { key_access_token, key_refresh_token } = await this.authCompanyService.findKeyByRefreshToken({
      cp_refresh_token
    })
    if (!key_access_token || !key_refresh_token) throw new HttpException('Token không hợp lệ', HttpStatus.UNAUTHORIZED)
    try {
      ...
      const inforCompany = await this.companiesService.getCompanyById({ id: dataToken[1]._id })
